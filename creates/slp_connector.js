...
const slp_connector_action_hidden = require("../triggers/slp_connector_action_hidden");
...
const perform = async (z, bundle) => {
  ...
  try {
    //Get the driver
    let selected_driver_response = await client.getDriver("slp-connector");
...
module.exports = {
  ...
  display: {
    label: "SLP Connector",
    description: "Configure actions using SLP Connector directly in Zapier.",
  },
    ...
    inputFields: [
      {
        ...
        dynamic: "slp_connector_action_hidden.key",
      },
      async function (z, bundle) {
        const client = new NexusClient();
        try {
          let response = await client.getDriver("slp-connector");
